...
class TestObservationBufferInitialization:
    ...
    @pytest.mark.parametrize("required_keys", [["rgb"], ["rgb", "depth", "proprio"]])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                0,
                pytest.raises(
                    ValueError, match=re.escape("buffer_size must be >= 1, got 0")
                ),
            ),
            (
                -3,
                pytest.raises(
                    ValueError, match=re.escape("buffer_size must be >= 1, got -3")
                ),
            ),
        ],
    )
...
class TestObservationBufferAdd:
    def test_add_accumulates_observations(self, observation_buffer_factory):
        buffer = observation_buffer_factory(buffer_size=3, required_keys=["rgb"])
        buffer.add(observations={"rgb": "frame_0"})
        buffer.add(observations={"rgb": "frame_1"})
        recent = buffer.get_recent()
        assert recent["rgb"] == ["frame_0", "frame_1"]

    def test_add_raises_on_missing_required_key(self, observation_buffer_factory):
        buffer = observation_buffer_factory(
        ...
        with pytest.raises(
            ValueError,
            match=re.escape(
                "Missing required observation key 'depth'. Got keys: ['rgb']"
            ),
        ):
            buffer.add(observations={"rgb": "frame_0"})

    def test_add_ignores_extra_keys(self, observation_buffer_factory):
        buffer = observation_buffer_factory(buffer_size=2, required_keys=["rgb"])
    ...
    def test_eviction_preserves_all_required_keys(self, observation_buffer_factory):
        buffer = observation_buffer_factory(
            buffer_size=2, required_keys=["rgb", "depth"]
        )
...
class TestObservationBufferIsReady:
    ...
    def test_buffer_size_one_ready_after_single_add(self, observation_buffer_factory):
        buffer = observation_buffer_factory(buffer_size=1, required_keys=["rgb"])
        buffer.add(observations={"rgb": "frame_0"})
        assert buffer.is_ready()
...
class TestObservationBufferGetRecent:
    def test_get_recent_returns_last_n_observations(self, observation_buffer_factory):
        buffer = observation_buffer_factory(buffer_size=4, required_keys=["rgb"])
        for index in range(4):
            buffer.add(observations={"rgb": f"frame_{index}"})
        recent = buffer.get_recent(count=2)
        assert recent["rgb"] == ["frame_2", "frame_3"]

    def test_get_recent_defaults_to_buffer_size(self, observation_buffer_factory):
        buffer = observation_buffer_factory(buffer_size=3, required_keys=["rgb"])
    ...
    def test_get_recent_with_count_larger_than_buffered(
        ...
        buffer = observation_buffer_factory(buffer_size=5, required_keys=["rgb"])
        buffer.add(observations={"rgb": "frame_0"})
        buffer.add(observations={"rgb": "frame_1"})
    ...
    def test_get_recent_with_count_zero(self, observation_buffer_factory):
