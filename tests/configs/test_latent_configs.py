...
class TestVAETransformerEncoderConfig:
    ...
    def test_config_has_correct_target(self):
        config = VAETransformerEncoderConfig(latent_dim=32, output_dim=512)
...
class TestGaussianPriorConfig:
    ...
    def test_config_has_correct_target(self):
        config = GaussianPriorConfig(latent_dim=32, output_dim=512)
    ...
    def test_config_instantiates_correctly(self):
        config = GaussianPriorConfig(latent_dim=32, output_dim=512, device="cpu")
...
class TestDiffusionPriorConfig:
    ...
    def test_config_has_correct_target(self):
        config = DiffusionPriorConfig(latent_dim=32, conditioning_dim=128, output_dim=512)
        assert config._target_ == "refactoring.models.decoding.latent.diffusion_prior.DiffusionPrior"

    def test_config_instantiates_correctly(self):
        config = DiffusionPriorConfig(latent_dim=32, conditioning_dim=128, output_dim=512, device="cpu")
        prior = instantiate(config)
