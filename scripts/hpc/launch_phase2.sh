...
task_ov() {
  case "$1" in
    circle)       echo "task/dataset_schema=synthetic/circle" ;;
    sequential)   echo "task/dataset_schema=synthetic/sequential" ;;
    corridor_k8)  echo "task/dataset_schema=synthetic/corridor_navigation task.dataset_schema.num_modes=8" ;;
    corridor_k16) echo "task/dataset_schema=synthetic/corridor_navigation task.dataset_schema.num_modes=16" ;;
    radial_k16)   echo "task/dataset_schema=synthetic/radial task.dataset_schema.num_modes=16" ;;
  esac
}
TASKS="circle sequential corridor_k8 corridor_k16 radial_k16"
SEEDS="42 43 44"
  ...
  for SEED in \$SEEDS; do
    for W in 0.001 0.01; do
      for TASK in \$TASKS; do
        T_OV=\$(task_ov \$TASK)
        \$TRAIN --config-name \$CFG \$T_OV experiment.seed=\$SEED \\
