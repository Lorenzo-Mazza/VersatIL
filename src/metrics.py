...
def compute_entropy(predicted_probability, eps=1e-8):
    ...
    return -torch.sum(
        predicted_probability * torch.log(predicted_probability + eps), dim=-1
    )
...
class ActEpochMetrics(EpochMetrics):
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        if self.binary_cross_entropy is not None:
            dict_to_return[Metrics.BINARY_CROSS_ENTROPY.value] = get_item(
    ...
    def accumulate(self, other: "ActEpochMetrics"):
        ...
        self.sinkhorn_loss = to_scalar(self.sinkhorn_loss) + to_scalar(
            other.sinkhorn_loss
        )
        ...
        self.kl_divergence = to_scalar(self.kl_divergence) + to_scalar(
            other.kl_divergence
        )
        if (
            self.binary_cross_entropy is not None
            and other.binary_cross_entropy is not None
        ):
            self.binary_cross_entropy = to_scalar(
                self.binary_cross_entropy
            ) + to_scalar(other.binary_cross_entropy)
...
class PhaseActEpochMetrics(ActEpochMetrics):
    ...
    def add_phase_predictions(
        self, predictions: torch.Tensor, labels: torch.Tensor, is_pad: torch.Tensor
    ):
    ...
    def compute_confusion_matrix(self):
        ...
        return confusion_matrix(
            self.phase_labels, self.phase_predictions, labels=list(range(self.n_phases))
        )
    ...
    def accumulate(self, other: "PhaseActEpochMetrics"):
        ...
        self.phase_cross_entropy = to_scalar(self.phase_cross_entropy) + to_scalar(
            other.phase_cross_entropy
        )
        self.phase_entropy = to_scalar(self.phase_entropy) + to_scalar(
            other.phase_entropy
        )
