...
class InferenceClient(AbstractModelClient):
    def __init__(
        ...
        **kwargs,
        ...
        update_rate_hz = (
            self.config.update_rate_hz if update_rate_hz is None else update_rate_hz
        )
        super().__init__(
            observation_buffer_size=self.config.obs_horizon,
            request_depth=self.config.use_depth,
            request_rectified_images=self.config.use_rectified,
            request_gripper_state=self.config.predict_gripper_action,
            predicts_in_camera_frame=self.config.predict_in_camera_frame,
            predicts_delta=self.config.deltas_as_actions,
            obs_robot_frame=self.config.obs_robot_frame,
    ...
    def get_actions_from_model(self) -> list[Action]:
        ...
        if state_dim > 0:
            ...
            qpos_tensor = torch.tensor(qpos, dtype=torch.float32).unsqueeze(
                0
            )  # Shape: (1, observation_buffer_size, state_dim)
        else:
            qpos_tensor = None
        ...
        if self.request_depth:
            ...
            )  # Shape: (1, observation_buffer_size, 1, H, W)
        ...
        if self.config.image_norm_type == ImageNormalizationType.IMAGENET.value:
            ...
            left_tensors = [
                self.imagenet_norm_back(image=left, mean=mean, std=std)
                for left in left_tensors
            ]
            right_tensors = [
                self.imagenet_norm_back(image=right, mean=mean, std=std)
                for right in right_tensors
            ]

        left_imgs = torch.stack(left_tensors).unsqueeze(
            0
        )  # Shape: (1, observation_buffer_size, 3, H, W)
        right_imgs = torch.stack(right_tensors).unsqueeze(
            0
        )  # Shape: (1, observation_buffer_size, 3, H, W)
        print(
        ...
        if self.temporal_agg:
            ...
            print(
                f"[TIMING] Temporal aggregation took: {time.time() - temporal_agg_start:.6f} seconds"
            )

            raw_action = raw_action.cpu().detach().numpy()
            raw_position_action = raw_action[:3]
            raw_gripper_action = (
                raw_action[-1] if self.config.predict_gripper_action else None
            )
            ...
            robot_action = np.concatenate(
                (raw_position_action[:3], [0.0])
            )  # we don't predict roll
            gripper_action = (
                raw_gripper_action > 0.5 if self.config.predict_gripper_action else None
            )
            ...
            for i in range(self.config.action_horizon):
                ...
                gripper_action = (
                    raw_gripper_action > 0.5
                    if self.config.predict_gripper_action
                    else None
                )
                actions.append(
    ...
    def get_exponential_averaged_action(self) -> torch.Tensor:
        ...
        self.all_time_actions[
            [self.timestep], self.timestep : self.timestep + self.config.action_horizon
        ] = self.current_all_actions[:, :, : self.action_dim]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Diffusion Policy/Flow Matching Model Client"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model-server-port", type=int, default=5555, help="Port of the model server"
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
...
def main():
    ...
    if device == torch.device("cpu"):
        print(
            "Warning: Running on CPU, this may be slow or go OOM. Consider using a GPU for better performance."
        )
