...
def rechunk_recompress_array(group, name,
    ...
    if (chunks == old_arr.chunks) and (compressor == (old_arr.compressors[-1] if old_arr.compressors else None)):
        # no change
        return old_arr
    # rechunk recompress
...
class ReplayBuffer:
    ...
    def create_empty_zarr(cls, storage = None, root = None):
        ...
        if 'episode_ends' not in meta:
            episode_ends = meta.create_array('episode_ends', shape=(0,), dtype=np.int64, compressors=None)
            ...
            for key in keys:
                ...
                if cks == value.chunks and cpr == value_cpr:
                    ...
                    n_copied, n_skipped, n_bytes_copied = zarr.copy(
                        ...
                        chunks=cks, compressors=cpr, if_exists=if_exists
                    )
    ...
    def _resolve_array_compressor(cls,
                                  compressors: Union[dict, str, BloscCodec], key, array):
