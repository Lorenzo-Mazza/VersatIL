...
from legacy_constants import (Cameras, ROBOT_FRAME_OBS_KEY, CAMERA_FRAME_OBS_KEY, GRIPPER_STATE_OBS_KEY, PHASE_LABEL_KEY)
...
def create_replay_buffer(
    ...
    crop_transform = CenterCrop(height=center_crop_size, width=center_crop_size) if center_crop else None
    resizer = Resize(height=image_height, width=image_width)
    depth_resizer = Resize(height=image_height, width=image_width, interpolation=cv2.INTER_NEAREST)

    data_group.create_array(ROBOT_FRAME_OBS_KEY, shape=(0, 3), chunks=(100, 3), dtype=np.float32,  compressors=[compressor])
    data_group.create_array(CAMERA_FRAME_OBS_KEY, shape=(0, 3), chunks=(100, 3), dtype=np.float32,  compressors=[compressor])
    if predict_gripper_action:
        data_group.create_array(GRIPPER_STATE_OBS_KEY, shape=(0, 1), chunks=(100, 1), dtype=np.float32,  compressors=[compressor])
    
    if task_has_phases:
        data_group.create_array(PHASE_LABEL_KEY, shape=(0, 1), chunks=(100, 1), dtype=np.uint8,  compressors=[compressor])
    ...
    for cam in camera_names:
        if cam == Cameras.DEPTH.value:
            ...
            dtype = np.uint8
        data_group.create_array(cam, shape=(0, *shape_suffix), chunks=(10, *shape_suffix), dtype=dtype, compressors=[compressor])
    ...
    with threadpool_limits(1):
        for path in sorted(dataset_paths, key=lambda x: int(Path(x).parent.name)):
            ...
            # Append positions
            data_group[ROBOT_FRAME_OBS_KEY].append(df[ROBOT_FRAME_KINEMATICS_COLS].values.astype(np.float32))
            data_group[CAMERA_FRAME_OBS_KEY].append(df[CAMERA_FRAME_KINEMATICS_COLS].values.astype(np.float32))
            
            if predict_gripper_action:
                data_group[GRIPPER_STATE_OBS_KEY].append(df[GRIPPER_STATE_COL].values[:, np.newaxis].astype(np.float32))            
            if task_has_phases:
                data_group[PHASE_LABEL_KEY].append(df[PHASE_LABEL_COL].values[:, np.newaxis].astype(np.uint8))

            left_image_col = LEFT_IMAGE_PATH_KEY if not use_rectified_images else RECTIFIED_LEFT_IMAGE_PATH_KEY
            right_image_col = RIGHT_IMAGE_PATH_KEY if not use_rectified_images else RECTIFIED_RIGHT_IMAGE_PATH_KEY
            key_to_sub = "framesLeft" if not use_rectified_images else "framesLeftRectified"
            # Append images for selected cameras
            for cam in camera_names:
                if cam == Cameras.DEPTH.value:
                    col = Cameras.DEPTH.value
                    df[col] = df[left_image_col].apply(lambda x: re.sub(r'(\d+)\.png$', r'depth_\1.npy', x.replace(key_to_sub, "depth")))
                else:
                    col = {
    ...
    meta_group.create_array('episode_ends', data=np.array(episode_ends), chunks=(len(episode_ends),), compressors=None)
