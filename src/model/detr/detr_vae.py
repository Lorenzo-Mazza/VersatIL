...
from model.common.dformerv2 import DFormerv2_S
...
class DETRVAE(nn.Module):
    def __init__(
        ...
        self.position_action_head = nn.Sequential(
    ...
    def forward(
        ...
        decoder_output = self.transformer(
            ...
            0
        ]  # (num_queries, B, hidden)

        predicted_actions = self.position_action_head(decoder_output)
        predicted_gripper_actions = None
        if self.predict_gripper_action:
            predicted_gripper_actions = self.gripper_action_head(decoder_output)
