...
from model.detr.transformer import (
    build_transformer,
...
class PhaseDETRVAE(nn.Module):
    def __init__(
        ...
        self.phase_temperature = nn.Parameter(
            torch.tensor(phase_learnable_temperature, dtype=torch.float32),
            requires_grad=True,
        )
        ...
        position_action_dimension = (
            action_dimension - 1 if predict_gripper_action else action_dimension
        )
        ...
        if Cameras.DEPTH.value in camera_names:
            ...
            self.fusion_proj = nn.Conv2d(
                hidden_dimension * 2, hidden_dimension, kernel_size=1
            )

            if depth_fusion_strategy == DepthFusionStrategy.ATTENTION.value:
                self.attention_fusion = nn.TransformerEncoder(
                    nn.TransformerEncoderLayer(
                        d_model=hidden_dimension, nhead=8, batch_first=True
                    ),
                    num_layers=1,
                )
                self.spatial_pool = SpatialSoftmax2d(
                ...
                self.position_encoding = build_position_encoding(
                    hidden_dim=hidden_dimension, position_embedding=position_embedding
                )
                ...
                self.dformer_pos = build_position_encoding(
                    hidden_dim=hidden_dimension, position_embedding=position_embedding
                )
                self.dformer_proj = nn.Conv2d(
                    self.dformer_encoder.backbone.num_features,
                    hidden_dimension,
                    kernel_size=1,
                )
        ...
        self.class_token_embedding = nn.Embedding(
            1, hidden_dimension
        )  # CLS token for encoder.
        self.vae_position_action_proj = nn.Linear(
            position_action_dimension, hidden_dimension
        )  # Actions to embed.
        if self.predict_gripper_action:
            self.vae_gripper_action_proj = nn.Linear(
                1, hidden_dimension
            )  # Gripper action to embed.
            self.encoder_action_proj_resized = nn.Linear(
                hidden_dimension * 2, hidden_dimension
            )
        self.vae_robot_state_proj = (
            nn.Linear(obs_dimension, hidden_dimension) if self.use_robot_state else None
        )  # qpos to embed.
        self.latent_proj = nn.Linear(
            hidden_dimension, self.latent_dim * 2
        )  # To mu/logvar.
        # Pos encodings for encoder input: CLS + [qpos] + actions.
        extra_slots = 1 + (1 if self.use_robot_state else 0) + chunk_size
        self.register_buffer(
            "pos_table", get_sinusoid_encoding_table(extra_slots, hidden_dimension)
        )
    ...
    def forward(
        ...
        if is_training:
            ...
            action_embeddings = self.vae_position_action_proj(
                position_actions
            )  # (B, seq, hidden)
            if self.predict_gripper_action:
                gripper_action_embeddings = self.vae_gripper_action_proj(
                    gripper_actions
                )  # (B, seq, hidden)
                action_embeddings = torch.cat(
                    [action_embeddings, gripper_action_embeddings], axis=-1
                )
                action_embeddings = self.encoder_action_proj_resized(action_embeddings)

            class_token_embedding = self.class_token_embedding.weight.unsqueeze(
                0
            ).repeat(
                batch_size, 1, 1
            )  # (B, 1, hidden)
            ...
            if self.use_robot_state:
                robot_state_embedding = self.vae_robot_state_proj(
                    robot_state
                ).unsqueeze(
                    1
                )  # (B, 1, hidden)
                encoder_input = torch.cat(
                    [class_token_embedding, robot_state_embedding, action_embeddings],
                    dim=1,
                )  # (B, 1+1+seq, hidden)
                non_action_pad_mask = torch.full(
                    (batch_size, 2), False, device=self.device
                )  # No pad for class token+robot state.
            else:
                encoder_input = torch.cat(
                    [class_token_embedding, action_embeddings], dim=1
                )  # (B, 1+seq, hidden)
                non_action_pad_mask = torch.full(
                    (batch_size, 1), False, device=self.device
                )

            # Transpose for transformer: (seq+1/2, B, hidden)
            encoder_input = encoder_input.permute(1, 0, 2)
            encoder_pad_mask = torch.cat(
                [non_action_pad_mask, is_pad], dim=1
            )  # (B, seq+)

            # Add pos encodings.
            encoder_positional_embedding = self.pos_table.clone().permute(
                1, 0, 2
            )  # (seq+, 1, hidden)
        ...
        for cam in self.camera_names:
            ...
            image_features[cam] = self.image_feature_proj(
                features[0]
            )  # Get last layer and project to hidden dimension.
        ...
        if has_depth:
            if self.depth_fusion_strategy == DepthFusionStrategy.ATTENTION.value:
                ...
                fused_image_features = fused_feat.unsqueeze(2).unsqueeze(
                    3
                )  # (B, hidden, 1, 1)
                fused_positional_embeddings = self.position_encoding(
                ...
                fused_left_features = torch.cat(
                    [
                        image_features[Cameras.LEFT.value],
                        image_features[Cameras.DEPTH.value],
                    ],
                    dim=1,
                )
                fused_left_features = self.fusion_proj(fused_left_features)
                fused_image_features = torch.cat(
                    [fused_left_features, image_features[Cameras.RIGHT.value]], dim=3
                )
                fused_positional_embeddings = torch.cat(
                    [
                        all_pos_embeddings[Cameras.LEFT.value],
                        all_pos_embeddings[Cameras.RIGHT.value],
                    ],
                    dim=3,
                )
                ...
                for cam in self.camera_names:
                    feat = (
                        image_features[cam].flatten(2).transpose(1, 2)
                    )  # (B, H*W, hidden)
                ...
                other_cams = [
                    ...
                    if c not in [Cameras.LEFT.value, Cameras.DEPTH.value]
                ]
                ...
                fused_map = self.dformer_encoder(
                    rgb_left, depth, return_feature_map=True
                )  # (B, C, Hs, Ws)
                fused_map = self.dformer_proj(fused_map)  # Project to hidden_dim
                fused_pos = self.dformer_pos(
                    fused_map
                )  # Positional encoding for fused map
                ...
                fused_image_features = torch.cat(
                    list(image_features.values()), dim=3
                )  # (B, hidden, H, W_total)
                fused_positional_embeddings = torch.cat(
                    list(all_pos_embeddings.values()), dim=3
                )
        else:
        ...
        decoder_output = self.transformer(
            input_features=fused_image_features,
            ...
            additional_positional_embedding=self.additional_pos_embed.weight,
        )[
            0
        ]  # (B, num_queries, hidden)

        # Phase prediction: pool decoder_output to get per-batch features
        phase_logits = (
            self.phase_head(decoder_output) / self.phase_temperature
        )  # (B, num_queries, n_phases)
        phase_probs = F.softmax(
            phase_logits, dim=-1
        )  # Apply temperature for learning the phase prediction confidence
        predicted_positions = [
            head(decoder_output) for head in self.position_action_heads
        ]  # List[n_phases] of (B, num_queries, 3)
        predicted_gripper_actions = None
        if self.predict_gripper_action:
            predicted_gripper_actions = [
                head(decoder_output) for head in self.gripper_action_heads
            ]  # List[n_phases] of (B, num_queries, 1)
