...
def add_positional_embedding(tensor: torch.Tensor, positional_embedding: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class PositionalEncoding(nn.Module):
  ...
  def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
    ...
    positional_encodings_slice = positional_encodings_slice.repeat((1, input_tensor.shape[1], 1))
...
class TimestepEmbeddingNetwork(nn.Module):
  ...
  def __init__(self, timestep_embedding_dim: int, output_dim: int, learnable_frequencies: bool = False) -> None:
    ...
    self.register_parameter(
        "frequencies", nn.Parameter(frequencies, requires_grad=learnable_frequencies)
    )
    self.output_network = nn.Sequential(
        nn.Linear(timestep_embedding_dim, output_dim), nn.SiLU(), nn.Linear(output_dim, output_dim)
    )
...
class SelfAttentionEncoderBlock(nn.Module):
  ...
  def forward(self, source_tensor: torch.Tensor, positional_embedding: torch.Tensor) -> torch.Tensor:
    ...
    feedforward_tensor = self.linear2(self.dropout2(self.activation(self.linear1(source_tensor))))
...
class ShiftScaleModulation(nn.Module):
  ...
  def forward(self, input_tensor: torch.Tensor, conditioning_vector: torch.Tensor) -> torch.Tensor:
    """Applies modulation.
...
class DiTDecoderBlock(nn.Module):
  ...
  def forward(self, input_tensor: torch.Tensor, timestep_embedding: torch.Tensor, condition_tensor: torch.Tensor) -> torch.Tensor:
    ...
    normalized_input = self.attention_mod1(self.norm1(input_tensor), combined_condition)
    attended_tensor, _ = self.self_attention(
        normalized_input, normalized_input, normalized_input, need_weights=False
    )
    input_tensor = self.attention_mod2(self.dropout1(attended_tensor), combined_condition) + input_tensor

    # MLP branch with modulation.
    normalized_input = self.mlp_mod1(self.norm2(input_tensor), combined_condition)
    feedforward_tensor = self.linear2(self.dropout2(self.activation(self.linear1(normalized_input))))
    feedforward_tensor = self.mlp_mod2(self.dropout3(feedforward_tensor), combined_condition)
...
class TransformerEncoder(nn.Module):
  ...
  def forward(self, source_tensor: torch.Tensor, positional_embedding: torch.Tensor) -> List[torch.Tensor]:
...
class TransformerDecoder(TransformerEncoder):
  ...
  def forward(self, source_tensor: torch.Tensor, timestep_embedding: torch.Tensor, all_condition_tensors: List[torch.Tensor]) -> torch.Tensor:
...
class DiTNoisePredictionNetwork(nn.Module):
  ...
  def __init__(
    ...
    self.register_parameter(
        ...
        nn.Parameter(torch.empty(prediction_horizon, 1, hidden_dim), requires_grad=True),
    )
  ...
  def forward(self, noisy_actions: torch.Tensor, timesteps: torch.Tensor, observation_encoding: torch.Tensor, encoder_cache: Optional[List[torch.Tensor]] = None) -> tuple[List[torch.Tensor], torch.Tensor]:
    ...
    return encoder_cache, self.forward_decoder(noisy_actions, timesteps, encoder_cache)
  ...
  def forward_decoder(self, noisy_actions: torch.Tensor, timesteps: torch.Tensor, encoder_cache: List[torch.Tensor]) -> torch.Tensor:
    ...
    action_tokens = action_tokens.transpose(0, 1)  # To (prediction_horizon, batch_size, hidden_dim)
    ...
    return self.noise_output_layer(decoder_output, timestep_embedding, encoder_cache[-1])
...
class DiTPolicy(nn.Module):
  ...
  def __init__(
    ...
    self.num_tokens = effective_images_per_camera * num_cameras * visual_feature_extractor.n_tokens
    ...
    self.token_post_processor = nn.Sequential(linear_projection, normalization_layer, nn.Dropout(dropout))
  ...
  def forward(self, images: Dict[str, torch.Tensor], proprio_observations: torch.Tensor, flattened_actions: torch.Tensor, flattened_masks: torch.Tensor) -> torch.Tensor:
    ...
    action_masks = flattened_masks.reshape((batch_size, self.prediction_horizon, self.action_dim))
    actions = flattened_actions.reshape((batch_size, self.prediction_horizon, self.action_dim))
    ...
    _, noise_prediction = self.noise_prediction_network(noisy_actions, timesteps, observation_tokens)

    # Compute masked MSE loss (only on valid actions).
    loss = nn.functional.mse_loss(noise_prediction, noise, reduction="none")
    loss = (loss * action_masks).sum(1)  # Sum over horizon and dim, average over batch later.
    return loss.mean()

  def get_actions(self, images: Dict[str, torch.Tensor], proprio_observations: torch.Tensor, num_steps: Optional[int] = None) -> torch.Tensor:
    ...
    observation_tokens = self.tokenize_observations(images, proprio_observations)
    encoder_cache = None
    noisy_actions = torch.randn(batch_size, self.prediction_horizon, self.action_dim, device=device)

    # Set diffusion steps for evaluation.
    eval_steps = self.evaluation_diffusion_steps
    if num_steps is not None:
      assert num_steps <= self.training_diffusion_steps, f"Can't exceed {self.training_diffusion_steps} steps."
      eval_steps = num_steps

    # Precompute encoder cache once.
    encoder_cache = self.noise_prediction_network.forward_encoder(observation_tokens)
  ...
  def tokenize_observations(self, images: Dict[str, torch.Tensor], proprio_observations: torch.Tensor, flatten: bool = False) -> torch.Tensor:
    ...
    if self.proprio_strategy == "add_token":
      ...
      processed_proprio = processed_proprio[:, None].repeat((1, image_tokens.shape[1], 1))
  ...
  def embed_images(self, images: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    """
    def embed_helper(extractor: nn.Module, image_tensor: torch.Tensor) -> torch.Tensor:
      ...
      if self.early_fusion and len(image_tensor.shape) == 5:
        ...
        image_tensor = torch.cat([image_tensor[:, t] for t in range(time_steps)], 1)
      ...
      # If temporal but not early_fusion, flatten batch*time and reshape back.
        ...
        embeds = extractor(image_tensor.reshape((batch_size * time_steps, channels, height, width)))
