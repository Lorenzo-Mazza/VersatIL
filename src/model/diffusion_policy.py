...
from legacy_constants import SHAPE_KEY, ACTION_KEY, POSITION_ACTION_KEY, ROBOT_STATE_KEY, OBSERVATION_KEY, DiffusionArchitecture, IS_PAD_KEY, ExplanationType, \
    GRIPPER_ACTION_KEY
...
class DiffusionPolicy(nn.Module):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], device: torch.device) -> torch.Tensor:
        ...
        if isinstance(self.model, TransformerForDiffusion):
    ...
    def compute_loss(self, batch, is_train: Optional[bool]=None, gripper_positive_class_weight: Optional[torch.Tensor]=None) -> DiffusionFlowMetrics:
        ...
        # Predict the residual noise using global conditioning.
        ...
        all_mse = Metrics.MSE_LOSS.to_metric()(target, pred, reduction='none')
    ...
    def conditional_sample(self, global_cond, device, dtype):
        ...
        for t in self.noise_scheduler.timesteps:
            ...
            trajectory = self.noise_scheduler.step(model_output, t, trajectory, **self.kwargs).prev_sample
        return trajectory
    ...
    def explain_predictions(self, explanation_types: list[str], obs_dict: Dict[str, torch.Tensor]) -> Dict[str, Dict[str, torch.Tensor]]:
        ...
        class DiffusionWrapper(nn.Module):
            ...
            def forward(self, observation, device):
                ...
                self.architecture.noise_scheduler.set_timesteps(self.architecture.num_inference_steps)
                for t in self.architecture.noise_scheduler.timesteps:
                    model_output = self.denoising_model(trajectory, t, global_cond=global_cond)
                    trajectory = self.architecture.noise_scheduler.step(model_output, t, trajectory, **self.architecture.kwargs).prev_sample
        ...
        model_to_explain = DiffusionWrapper(encoder=self.obs_encoder, denoising_model=self.model, n_obs_steps=self.n_obs_steps, architecture=self)
