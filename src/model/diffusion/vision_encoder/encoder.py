...
class DepthResNet(nn.Module):
    ...
    def __init__(self, backbone = 'resnet18', num_classes = 512, use_group_norm = True, pretrained = True):
        super().__init__()

        # Load pretrained RGB model
        base_model = get_resnet(backbone, weights="IMAGENET1K_V1" if pretrained else None)

        # Adapted conv1 for 1 channel
        self.conv1 = nn.Conv2d(1, base_model.conv1.out_channels, kernel_size=base_model.conv1.kernel_size, stride=base_model.conv1.stride, padding=base_model.conv1.padding, bias=False)
        ...
        self.spatial_softmax = SpatialSoftmax2d(normalize=True, temperature=1.0, learnable_temperature=False)

        # Linear layer after pooling
        in_features = base_model.layer4[-1].conv2.out_channels * 2  # for action_embedding,y per channel
...
class RGBResNet(nn.Module):
    ...
    def __init__(self, backbone = 'resnet18', num_classes = 512, input_channels = 3, use_group_norm = True, pretrained = False):
        super().__init__()

        base_model = get_resnet(backbone, weights="IMAGENET1K_V1" if pretrained else None)

        self.conv1 = nn.Conv2d(input_channels, base_model.conv1.out_channels, kernel_size=base_model.conv1.kernel_size, stride=base_model.conv1.stride, padding=base_model.conv1.padding, bias=False)
        if pretrained:
            original_weight = base_model.conv1.weight.data
            if input_channels == 3:
                ...
                self.conv1.weight.data[:, 3:] = original_weight[:, :remaining % 3].repeat(1, remaining // 3 + 1, 1, 1)[:, :remaining]
...
class MultiImageObsEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for key, shape in obs_shapes.items():
            if len(shape) == 3:  # Image
                ...
                if resize_shape:
                    h, w = resize_shape.get(key, resize_shape) if isinstance(resize_shape, dict) else resize_shape
                    this_resizer = torchvision.transforms.Resize(size=(h, w))
                    input_shape = (shape[0], h, w)

                this_randomizer = nn.Identity()
                if crop_shape:
                    h, w = crop_shape.get(key, crop_shape) if isinstance(crop_shape, dict) else crop_shape
                    this_randomizer = CropRandomizer(input_shape=input_shape, crop_height=h, crop_width=w, num_crops=1,
                                                     pos_enc=False) if random_crop else torchvision.transforms.CenterCrop(size=(h, w))
                ...
                self.key_transforms[key] = nn.Sequential(this_resizer, this_randomizer, this_normalizer)
                # Skip encoder creation for fused keys in 'dformer'
                if self.depth_fusion_strategy == DepthFusionStrategy.GEOMETRIC_ATTENTION.value and key in [Cameras.LEFT.value, Cameras.DEPTH.value]:
                    continue

                if key == Cameras.DEPTH.value:
                    self.key_encoders[key] = DepthResNet(backbone=backbone, use_group_norm=use_group_norm, pretrained=pretrained)
                else:
                    self.key_encoders[key] = RGBResNet(backbone=backbone, input_channels=shape[0], use_group_norm=use_group_norm, pretrained=pretrained)
        ...
        if self.key_encoders:
            ...
            self.map_dim = first_encoder.backbone.layers[-1].embed_dim  # Last stage channels in DFormerv2
            self.feature_dim = first_encoder.get_output_dim()  # Custom method in DFormerDepthEncoder

        if depth_fusion_strategy == DepthFusionStrategy.LEFT_CHANNEL_WISE.value:
            if Cameras.LEFT.value not in self.image_keys:
                raise ValueError("Left-channel depth fusion requires depth and left rgb key.")
        ...
        if depth_fusion_strategy == DepthFusionStrategy.ATTENTION.value:
            self.attention_fusion = nn.TransformerEncoder(
                nn.TransformerEncoderLayer(d_model=self.feature_dim, nhead=8, batch_first=True),
                num_layers=1
            )
        if self.depth_fusion_strategy == DepthFusionStrategy.GEOMETRIC_ATTENTION.value:
            if Cameras.LEFT.value not in self.image_keys:
                raise ValueError("Geometric attention depth fusion requires depth and left rgb key.")
    ...
    def forward(self, obs_dict, is_train: Optional[bool] = None):
        ...
        B, T = obs_dict[self.image_keys[0]].shape[:2] if is_sequence else (obs_dict[self.image_keys[0]].shape[0], None)
        flat_dim = (B * T) if is_sequence else B
        cat_dim_spatial = 4 if is_sequence else 3  # For feat_maps: seq (B,T,C,H,W), cat width=4; non-seq (B,C,H,W), cat=3
        cat_dim_channel = 2 if is_sequence else 1

        if self.depth_fusion_strategy == DepthFusionStrategy.GEOMETRIC_ATTENTION.value:
            # Fuse left RGB + Depth using DFormer's geometric attention encoding
            if Cameras.LEFT.value not in self.image_keys:
                raise ValueError("Geometric attention fusion requires depth and left rgb key.")
            ...
            depth = self.key_transforms[Cameras.DEPTH.value](depth) if is_train else depth
            ...
            other_keys = [k for k in self.image_keys if k not in [Cameras.LEFT.value, Cameras.DEPTH.value]]
        ...
        elif self.depth_fusion_strategy == DepthFusionStrategy.SEPARATE.value or self.depth_fusion_strategy is None:
            ...
            for key in self.image_keys:
                ...
                if is_sequence:
                    img = img.view(flat_dim, *img.shape[2:])
                img = self.key_transforms[key](img) if is_train else img
                feat_map = self.key_encoders[key](img, return_feature_map=True)  # (flat_dim,512,H',W')
                if is_sequence:
            ...
            # Fuse primary + depth
            fused_primary = torch.cat([feat_maps[Cameras.LEFT.value], feat_maps[Cameras.DEPTH.value]], dim=cat_dim_channel)
            if is_sequence:
            ...
            remaining_keys = [k for k in self.image_keys if k not in [Cameras.LEFT.value, Cameras.DEPTH.value]]
            fused_map = torch.cat([fused_primary] + [feat_maps[k] for k in remaining_keys], dim=cat_dim_spatial)
            if is_sequence:
                fused_map = fused_map.view(flat_dim, *fused_map.shape[2:])
            ...
            stacked = torch.cat(view_feats, dim=-2)  # (B,T,num_views,512) or (B,num_views,512)
            if is_sequence:
    ...
    def get_output_dim(self):
        ...
        if self.depth_fusion_strategy in [DepthFusionStrategy.LEFT_CHANNEL_WISE.value, DepthFusionStrategy.ATTENTION.value, DepthFusionStrategy.WIDTH.value]:
            ...
            return (self.feature_dim * (rgb_count + (1 if has_depth else 0))) + state_dim