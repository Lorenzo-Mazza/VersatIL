...
def get_depth_image_normalizer(
    ...
    input_std,
    device=None,
    norm_type: str = DepthNormalizationType.ZERO_TO_ONE.value,
):
    if norm_type in [
        DepthNormalizationType.ZERO_TO_ONE.value,
        DepthNormalizationType.IMAGENET.value,
    ]:
    ...
    stage1 = SingleFieldLinearNormalizer.create_manual(
        scale=scale, offset=offset, input_stats_dict=stat
    )
    ...
    if norm_type == DepthNormalizationType.IMAGENET.value:
        ...
        scaled_mean = output_min + (input_mean - input_min) / input_range * (
            output_max - output_min
        )
        scaled_std = input_std / input_range * (output_max - output_min)

        scale2 = torch.tensor(
            [1.0 / IMAGENET_DEPTH_STD], dtype=torch.float32, device=device
        )
        offset2 = torch.tensor(
            [-IMAGENET_DEPTH_MEAN / IMAGENET_DEPTH_STD],
            dtype=torch.float32,
            device=device,
        )
        stat2 = {
        ...
        stage2 = SingleFieldLinearNormalizer.create_manual(
            scale=scale2, offset=offset2, input_stats_dict=stat2
        )
...
if __name__ == "__main__":
    ...
    n_minus_one = get_depth_image_normalizer(
        dmin, dmax, dmean, dstd, norm_type="minus_one_to_one"
    )
    ...
    assert isinstance(
        n_minus_one, SingleFieldLinearNormalizer
    ), f"Expected SingleFieldLinearNormalizer, got {type(n_minus_one)}"
    assert torch.allclose(
        normalized.min(), torch.tensor([-1.0]), atol=1e-6
    ...
    assert torch.allclose(
        output_stats["min"], torch.tensor([-1.0])
    ...
    assert isinstance(
        n_imagenet, SequentialNormalizer
    ), f"Expected SequentialNormalizer, got {type(n_imagenet)}"
    assert (
        hasattr(n_imagenet, "normalizers") and n_imagenet.normalizers is not None
    ), "SequentialNormalizer should have normalizers"
    assert (
        len(n_imagenet.normalizers) == 2
    ...
    assert torch.allclose(
        output_stats["min"], torch.tensor([expected_min]), atol=1e-4
    ), f"Output min should be {expected_min:.4f}, got {output_stats['min'].item():.4f}"
    assert torch.allclose(
        output_stats["max"], torch.tensor([expected_max]), atol=1e-4
    ), f"Output max should be {expected_max:.4f}, got {output_stats['max'].item():.4f}"
    assert torch.allclose(
        output_stats["mean"], torch.tensor([expected_mean]), atol=1e-4
    ), f"Output mean should be {expected_mean:.4f}, got {output_stats['mean'].item():.4f}"
    assert torch.allclose(
        output_stats["std"], torch.tensor([expected_std]), atol=1e-4
    ...
    print(
        f"✓ Output stats: min={output_stats['min'].item():.4f}, max={output_stats['max'].item():.4f}"
    )
    print(
        f"✓ Output stats: mean={output_stats['mean'].item():.4f}, std={output_stats['std'].item():.4f}"
    )
    print(f"✓ Round-trip successful")
    ...
    assert isinstance(
        retrieved_zero, SingleFieldLinearNormalizer
    ...
    assert isinstance(
        retrieved_imagenet, SequentialNormalizer
    ...
    assert torch.allclose(
        normalized_zero.min(), torch.tensor([0.0]), atol=1e-6
    ...
    assert torch.allclose(
        data, unnormalized_imagenet, atol=1e-6
    ...
    assert isinstance(
        loaded_zero, SingleFieldLinearNormalizer
    ), "Loaded ZERO_TO_ONE type incorrect"
    assert isinstance(
        loaded_imagenet, SequentialNormalizer
    ...
    assert torch.allclose(
        orig_zero_out, loaded_zero_out, atol=1e-6
    ), "Loaded ZERO_TO_ONE produces different output"
    assert torch.allclose(
        orig_imagenet_out, loaded_imagenet_out, atol=1e-6
    ...
    if torch.cuda.is_available():
        ...
        n_cuda = get_depth_image_normalizer(
            dmin, dmax, dmean, dstd, device=device, norm_type="imagenet"
        )
        ...
        assert (
            normalized_cuda.device.type == "cuda"
        ), f"Output should be on CUDA, got {normalized_cuda.device}"
        assert (
            n_cuda.params_dict["scale"].device.type == "cuda"
