    ...
    def __setitem__(self, key: str, value):
        if isinstance(value, SequentialNormalizer):
            ...
            params_dict["_num_normalizers"] = nn.Parameter(
                torch.tensor([float(len(normalizer_list))])
            )
...
def test():
    ...
    offset2 = torch.tensor(
        [-imagenet_mean / imagenet_std], dtype=torch.float32, device="cpu"
    )
    normalizer1 = SingleFieldLinearNormalizer.create_manual(
        scale=scale1, offset=offset1, input_stats_dict=input_stat1
    )
    normalizer2 = SingleFieldLinearNormalizer.create_manual(
        scale=scale2, offset=offset2, input_stats_dict=input_stat2
    )
