...
class DFormerDepthEncoder(nn.Module):
    ...
    def __init__(
        ...
        **dformer_kwargs,
        ...
        # Build DFormer
        if variant.upper() == "S":
            self.backbone = DFormerv2_S(
                out_indices=out_indices, drop_path_rate=drop_path_rate, **dformer_kwargs
            )
        elif variant.upper() == "B":
            self.backbone = DFormerv2_B(
                out_indices=out_indices, drop_path_rate=drop_path_rate, **dformer_kwargs
            )
        elif variant.upper() == "L":
            self.backbone = DFormerv2_L(
                out_indices=out_indices, drop_path_rate=drop_path_rate, **dformer_kwargs
            )
        else:
    ...
    def forward(
        self, rgb: torch.Tensor, depth: torch.Tensor, return_feature_map: bool = False
    ) -> torch.Tensor:
        """
        rgb:   (B,3,H,W)
