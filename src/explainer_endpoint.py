...
class ModelExplainer:
    ...
    def get_observation(self) -> dict:
        ...
        for cam in self.config.camera_names:
            if cam == Cameras.DEPTH.value:
                ...
                df[col] = df[LEFT_IMAGE_PATH_KEY].apply(
                ...
                col = {
                    ...
                    if not self.config.use_rectified
            ...
            if cam == Cameras.DEPTH.value:
                disp = [
                    cv2.imread(img, cv2.IMREAD_UNCHANGED).astype(np.float32)
                    for img in df[col]
                ]
    ...
    def explain_prediction(
        ...
        left_tensor = observation[Cameras.LEFT.value].unsqueeze(
            0
        )  # Shape: (1, self.observation_horizon, 3, H, W)
        ...
        heatmaps = self.model.policy.explain_predictions(
            obs_dict=obs_dict, explanation_types=self.explanation_types
        )
                        ...
                        if self.observation_horizon > 1:
                            for i in range(self.observation_horizon):
                                ...
                                if channel == 3:
                                    ...
                                    normalized = (
                                        (obs_single[0] / obs_single[0].max())
                                        .cpu()
                                        .numpy()
                                    )
                                    img = np.repeat(
                                        normalized[:, :, np.newaxis], 3, axis=2
                                    )
                                cam_img = show_cam_on_image(
                                    img, heatmap_single, use_rgb=True, image_weight=0.5
                                )
                                ...
                                save_path = os.path.join(
                                    folder_path,
                                    f"timestep_{self.timestep}_sub_{i}_{explanation_type}_{cam}.png",
                                )
                                cv2.imwrite(save_path, cam_img_bgr)
                                print(
                                    f"Saved heatmap for timestep {self.timestep} sub {i} in {save_path}"
                                )
                        else:
                            heatmap_np = heatmap_np[
                                0
                            ]  # Remove the observation sequence dimension
                            ...
                            cam_img = show_cam_on_image(
                                img, heatmap_np, use_rgb=True, image_weight=0.5
                            )
                            cam_img_bgr = cv2.cvtColor(cam_img, cv2.COLOR_RGB2BGR)
                            folder_name = path.name
                            folder_path = os.path.join(self.save_dir, folder_name)
                            ...
                            save_path = os.path.join(
                                folder_path,
                                f"timestep_{self.timestep}_{explanation_type}_{cam}.png",
                            )
                            cv2.imwrite(save_path, cam_img_bgr)
                            print(
                                f"Saved heatmap for timestep {self.timestep} in {save_path}"
                            )
