...
class VLMEncoder(Encoder):
    ...
    def __init__(
        ...
        self.image_processor = AutoImageProcessor.from_pretrained(model_name, do_rescale = False, do_normalize = False,
                                                                  do_convert_rgb = False, do_center_crop = False,
                                                                  do_resize = False)
    ...
    def _setup_pooling(self):
        ...
        if self.pooling_method == PoolingMethod.NONE.value:
            self.output_vision_dim = (-1, self.hidden_vision_dim) # -1 indicates the variable sequence dimension
    ...
    def _pool_features(self, outputs: BaseModelOutputWithPooling, modality: str) -> torch.Tensor:
        ...
        if self.pooling_method == PoolingMethod.DEFAULT.value:
            ...
            raise ValueError(f"Unsupported feature extraction method: {self.pooling_method}")
    ...
    def _resize_images(self, images: torch.Tensor) -> torch.Tensor:
        ...
        padded_img = torch.nn.functional.pad(resized_img, (pad_width, 0, pad_height, 0), value=0.0)
        return padded_img
    ...
    def forward(self, inputs: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:  # type: ignore[override]
        ...
        image_features = self._pool_features(outputs.vision_model_output, modality=EncoderOutputKeys.RGB.value)
        language_features = self._pool_features(outputs.text_model_output, modality=EncoderOutputKeys.LANGUAGE.value)
        ...
        if has_time:
            if self.pooling_method == PoolingMethod.NONE.value:
                ...
                image_features = image_features.view(B, T, vision_seq_len, self.hidden_vision_dim)
                language_features = language_features.view(B, T, self.max_text_length, self.hidden_language_dim)
                ...
                language_features = language_features.view(B, T, self.hidden_language_dim)
                token_padding_mask = torch.zeros(B, T, dtype=torch.bool, device=image_features.device)
        else:
            if self.pooling_method != PoolingMethod.NONE.value:
                token_padding_mask = torch.zeros(B, dtype=torch.bool, device=image_features.device)
        return {EncoderOutputKeys.RGB.value: image_features,
