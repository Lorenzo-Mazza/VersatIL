    ...
    def forward(self, x:Tensor, cls_q:Tensor):
        if x.ndim == 4:
            ...
            if x.shape[1] == self.norm.normalized_shape[0]:
                ...
                raise ValueError(f"Input shape {x.shape} not compatible with AttentionPool2d "
        ...
        k, v = k[0], v[0]  # [B,N,C]
        attended_values = torch.nn.functional.scaled_dot_product_attention(query=q, key=k, value=v)  # [B,1,C]
