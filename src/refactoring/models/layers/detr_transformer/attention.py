...
class FlashAttention(nn.Module):
    def __init__(self, embedding_dimension: int, number_of_heads: int, dropout: float = 0.0):
        super().__init__()
        if embedding_dimension % number_of_heads != 0:
            raise ValueError("Attention layer embedding_dimension must be divisible by number_of_heads.")
    ...
    def forward(
        ...
        q = self.q_proj(add_positional_encoding(query, query_positional_encoding)) # (B, query_length, embedding_dimension)
        k = self.k_proj(add_positional_encoding(key, key_positional_encoding)) # (B, key_length, embedding_dimension)
        v = self.v_proj(value) # (B, key_length, embedding_dimension)
        q = q.view(B, query_length, self.number_of_heads, self.head_size).transpose(1, 2) # (B,query_length, nh, hs)
        k = k.view(B, key_length, self.number_of_heads, self.head_size).transpose(1, 2) # (B, key_length, nh, hs)
        v = v.view(B, key_length, self.number_of_heads, self.head_size).transpose(1, 2) # (B, key_length, nh, hs)
        mask = None
        if attention_mask is not None or key_padding_mask is not None:
            attn_bool = None
            if attention_mask is not None:
                attn_bool = attention_mask if attention_mask.dtype == torch.bool else torch.isneginf(attention_mask)
                if attn_bool.dim() == 2:
                    attn_bool = attn_bool.unsqueeze(0)          # (1, query_length, key_length)
            padding_bool = key_padding_mask.bool() if key_padding_mask is not None else None
            if attn_bool is not None and padding_bool is not None:
                mask = torch.zeros(B, query_length, key_length, dtype=torch.bool, device=query.device)
        ...
        y = y.transpose(1, 2).contiguous().view(B, query_length, C) # (B, query_length, embedding_dimension)
