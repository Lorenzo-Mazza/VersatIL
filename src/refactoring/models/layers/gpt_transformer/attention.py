...
from refactoring.models.layers.gpt_transformer.positional_encoding import apply_positional_encoding
...
class CachedAttention(nn.Module):
    ...
    def compute_query_key_value(
        ...
        # Keep K/V compact for GQA (repeat happens later, after caching)
    ...
    def forward(
        ...
        """
        # For cross-attention with precomputed K/V
        if use_cross_attention_cache and layer_cache is not None:
            ...
            # Now concatenate with cached K/V (which already have correct rotations)
            if layer_cache is not None and layer_cache.self_attention_keys.numel() > 0:
        ...
        # Update cache if requested (for self-attention only)
        new_cache = None
        if use_cache and not use_cross_attention_cache:
