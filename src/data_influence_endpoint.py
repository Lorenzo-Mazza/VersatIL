...
from constants import PolicyType, InfluenceMethod, ACTION_KEY, OBSERVATION_KEY, IS_PAD_KEY, ROBOT_STATE_KEY
...
from kfac import KFAC  # Assuming installed for Hessian approx; fallback to identity if not
...
class DataInfluenceEstimator:
    ...
    def _load_model(self) -> DiffusionWorkspace | FlowMatchingWorkspace | ACTWorkspace:
        ...
        workspace.load_checkpoint(path=f"{self.checkpoint_path}/best.pt")  # Or /latest.pt
        self.model = workspace
        return workspace


    def _subsample_loader(self, loader: DataLoader, subsample_size: int) -> DataLoader:
        dataset_len = len(loader.dataset) if hasattr(loader.dataset, '__len__') else loader.batch_size * len(loader)
    ...
    def _episode_to_batch(self, episode_data: Dict[str, np.ndarray]) -> Dict[str, torch.Tensor]:
        len_episode = len(episode_data[ACTION_KEY])
        batch = {OBSERVATION_KEY: {}, ACTION_KEY: torch.from_numpy(episode_data[ACTION_KEY]).unsqueeze(0).to(self.config.device),
                 IS_PAD_KEY: torch.zeros((1, len_episode), dtype=torch.bool).to(self.config.device)}
        for cam in self.config.camera_names:
            batch[OBSERVATION_KEY][cam] = torch.from_numpy(episode_data[cam]).unsqueeze(0).to(self.config.device)
        if ROBOT_STATE_KEY in episode_data:
            batch[OBSERVATION_KEY][ROBOT_STATE_KEY] = torch.from_numpy(episode_data[ROBOT_STATE_KEY]).unsqueeze(0).to(self.config.device)
        return batch
    ...
    def _get_average_gradient(self, loader: DataLoader, negate: bool = False) -> torch.Tensor:
        avg_grad = None
        count = 0
        for batch in loader:
    ...
    def compute_influences(self):
        ...
        # Save sorted scores
        sorted_scores = dict(sorted(self.influence_scores.items(), key=lambda x: x[1], reverse=True))
        np.savez(self.save_dir / f"{self.method}_scores.npz", **sorted_scores)
        print(f"Saved scores to {self.save_dir}. Top episodes: {list(sorted_scores.items())[:5]}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute data influences post-training")
    parser.add_argument(
        "--checkpoint-path",
        type=str,
