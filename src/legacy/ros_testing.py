...
def unpack_observations(
    obs: list[Float32MultiArray],
) -> tuple[list[tuple], list[tuple], list[PILImage], list[PILImage]]:
    ...
    for data in obs:
        ...
        left_frame = (
            PILImage.fromarray(left_frame).crop((210, 0, 750, 540)).resize((512, 512))
        )
...
class Model:
    def __init__(self, config: TaskBaseConfig):
        ...
        self.model.load_checkpoint(
            path="/mnt/cluster/workspaces/mazzalore/iros/experiment_20250404_190135/best_model.pt"
        )
    ...
    def get_action(
        ...
        left_img: list[PILImage],
        right_img: list[PILImage],
...
class Subscribers:
    def __init__(self, update_every_ms):
        ...
        self.camera_robot_transformation = [
            0.84248458,
            -0.03834644,
            -0.00607486,
            0.53630644,
    ...
    def update(self):
        ...
        relative_tip_pos = [
            robot_state.relativeTipPosition.x,
            robot_state.relativeTipPosition.y,
            robot_state.relativeTipPosition.z,
        ]
        self.rel_tip_pos = relative_tip_pos
        abs_tip_pos = [
            robot_state.tipPosition.x,
            robot_state.tipPosition.y,
            robot_state.tipPosition.z,
        ]
        camera_tip_pos = (
            np.array(abs_tip_pos) + np.dot(self.rotation, np.array(abs_tip_pos))
        ).tolist()
        ...
        left_frame.save(
            f"/mnt/cluster/temp/threading_il/current_observation/left_{self.counter}_left.png"
        )
        depth_left.save(
            f"/mnt/cluster/temp/threading_il/current_observation/depth_{self.counter}_depth.png"
        )
        right_frame.save(
            f"/mnt/cluster/temp/threading_il/current_observation/right_{self.counter}_right.png"
        )
        ...
        if self.counter > 1 and len(self.left_img_buffer) > 1:
            action = self.model.get_action(
                post=self.camera_tip_pos_buffer,
