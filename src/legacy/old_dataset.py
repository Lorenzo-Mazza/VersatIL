...
from model.common.normalize_util import (
    get_image_range_normalizer,
    get_depth_image_normalizer,
    get_zero_to_one_normalizer,
)
...
class EpisodicDataset(data.Dataset):
    def __init__(
        ...
        self.rgb_transform = A.Compose(
            ...
            + [
                ...
                if image_norm_type != ImageNormalizationType.IMAGENET.value
                else A.Normalize(
        ...
        for end in self.episode_ends:
            ep_indices = [
                i for i, row in enumerate(self.indices) if current_start <= row[0] < end
            ]
    ...
    def camera_name_to_col(self) -> Dict[str, str]:
        ...
        return {
            Cameras.LEFT.value: LEFT_IMAGE_PATH_KEY
            if not self.use_rectified_images
            else RECTIFIED_LEFT_IMAGE_PATH_KEY,
            Cameras.RIGHT.value: RIGHT_IMAGE_PATH_KEY
            if not self.use_rectified_images
    ...
    def load_dataset(self) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        for path in sorted(self.dataset_paths, key=lambda x: int(Path(x).parent.name)):
            ...
            if self.use_depth:
                episode_df[Cameras.DEPTH.value] = episode_df[LEFT_IMAGE_PATH_KEY].apply(
                    lambda x: x.replace("framesLeft", "stereoDepth").replace(
                        ".png", ".tiff"
                    )
                )
            if self.center_initial_position:
                ...
                episode_df[col_list] = (
                    episode_df[col_list].astype(float).sub(initial_pos, axis=1)
                )
            target_columns = (
                CAMERA_FRAME_KINEMATICS_COLS
                if self.camera_space_actions
                else ROBOT_FRAME_KINEMATICS_COLS
            )
            if self.deltas_as_actions:
                episode_df[ACTION_COL_KEYS] = np.nan_to_num(
                    np.array(episode_df[target_columns].shift(-1))
                    - np.array(episode_df[target_columns])
                )
            else:
                episode_df[ACTION_COL_KEYS] = (
                    episode_df[target_columns].shift(-1).ffill()
                )
    ...
    def compute_depth_stats(self):
        ...
        for image_path in self.dataset[Cameras.DEPTH]:
            ...
            depth_tensor = self.depth_transform(image=depth_image)["image"].unsqueeze(
                0
            )  # Apply image transform (resize, to tensor)
            ...
            depth_mean = (
                depth_mean * num_depth_images + new_depth_mean * batch_size
            ) / (num_depth_images + batch_size)
            depth_var = (depth_var * num_depth_images + new_depth_var * batch_size) / (
                num_depth_images + batch_size
            )
            num_depth_images += batch_size
    ...
    def get_normalizer(
        ...
        if self.use_kinematics:
            kinematics_data[ROBOT_STATE_KEY] = self.dataset[
                self.observation_cols
            ].values

        normalizer = LinearNormalizer()
        normalizer.fit(
            data=kinematics_data, last_n_dims=1, mode=mode, device=device, **kwargs
        )
    ...
    def sample_sequence(
        ...
        for key, input_arr in data.items():
            ...
            if sample_start_idx > 0:
                data_arr[:sample_start_idx] = (
                    input_arr[0] if not self.deltas_as_actions else 0.0
                )
            if sample_end_idx < sequence_length:
                data_arr[sample_end_idx:] = (
                    input_arr[-1] if not self.deltas_as_actions else 0.0
                )
    ...
    def __getitem__(self, episode_idx):
        if self.sampling_mode == SamplingMode.OVERLAPPING.value:
            ...
            (
                ...
                sample_start_idx,
                sample_end_idx,
            ) = self.indices[rand_ind]
            episode_start = (
                0 if episode_idx == 0 else self.episode_ends[episode_idx - 1]
            )
            buffer_start_idx_action = max(
                buffer_start_idx - self.action_backward_shift, episode_start
            )
            buffer_end_idx_action = buffer_start_idx_action + (
        ...
        for i in range(self.obs_horizon):
            for camera in self.camera_names:
                ...
                if camera == Cameras.DEPTH.value:
                    depth_np = self._read_image(
                        image_path=image_path, is_disparity=True
                    )
                    image_tensor = self.depth_transform(image=depth_np)[
                        "image"
                    ]  # Apply image transform (resize, to tensor)
                    image_tensor = (
                    ...
                    image_np = self._read_image(
                        image_path=image_path, is_disparity=False
                    )
                    if self.augmentation_transform and self.train:
                        image_np = self.augmentation_transform(image=image_np)[
                            "image"
                        ]  # Apply augmentation if needed
                    image_tensor = self.rgb_transform(image=image_np)[
                        "image"
                    ]  # Apply image transform (resize, to tensor)
        ...
        for camera in self.camera_names:
            sample[OBSERVATION_KEY][camera] = torch.stack(
                camera_to_image_tensors[camera]
            )
        if self.use_kinematics:
            sample[OBSERVATION_KEY][ROBOT_STATE_KEY] = torch.from_numpy(
                padded_sample[OBSERVATION_KEY][: self.obs_horizon, :]
            ).float()
        sample[ACTION_KEY] = torch.from_numpy(
            padded_sample[ACTION_KEY][: self.pred_horizon, :]
        ).float()
...
def get_dataloaders(config: ACTConfig):
    ...
    num_val_episodes = min(
        int(len(sorted_paths) * config.ratio_validation_episodes), len(sorted_paths) - 1
    )
    train_paths, val_paths = split_episodes(
        sorted_paths, num_val_episodes=num_val_episodes, seed=config.seed
    )
