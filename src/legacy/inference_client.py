...
def parse_args():
    ...
    parser.add_argument('--server_address', type=str, default='tcp://localhost:5555',
                        help='Address of the observation server')
    parser.add_argument('--update_interval', type=float, default=0.7,
                        help='Time interval between updates in seconds')
    return parser.parse_args()


def robot_to_camera_space(robot_space_pos, rotation) -> list[float]:
    return (np.array(robot_space_pos) + np.dot(rotation, np.array(robot_space_pos))).tolist()

def camera_to_robot_space(camera_space_pos, rotation) -> list[float]:
    return (np.array(camera_space_pos) + np.dot(rotation.T, np.array(camera_space_pos))).tolist()
...
class Model:
    ...
    def __init__(self, config: TaskBaseConfig):
        ...
        self.model.load_checkpoint(path="/mnt/cluster/workspaces/mazzalore/iros/diffusion_policy_checkpoints/experiment_20250515_113430/latest.pt")
    ...
    def get_action(self, post: list[tuple[float, float, float]], left_img: list, right_img: list,
                   depth_img: list):
...
class RosSubscriber:
    def __init__(self):
        ...
        self.socket = self.context.socket(zmq.REQ)  # REQ socket for request-reply pattern
        ...
        rospy.Subscriber("/ur5e_rcm_controller/robotStateRcm", RobotStateRcm, self.robot_state_callback)
        
        # Subscribe to camera images
        rospy.Subscriber("/laparoscope/camera/left/image_raw", Image, self.left_img_callback)
        rospy.Subscriber("/laparoscope/camera/right/image_raw", Image, self.right_img_callback)
    ...
    def robot_state_callback(self, msg):
        ...
        self.relative_tip_position_robot_space = [msg.relativeTipPosition.x, msg.relativeTipPosition.y, msg.relativeTipPosition.z]
        self.relative_tip_position_camera_space = robot_to_camera_space(self.relative_tip_position_robot_space, self.rotation)
        ...
        if self.model.predict_global_position:
            temp = np.array(self.relative_tip_position_camera_space) - np.array(self.initial_tip_pos_camera_space)
    ...
    def update_loop(self):
        ...
        try:
            ...
            if self.left_img is None or self.right_img is None or self.relative_tip_position_robot_space is None:
                return
            ...
            if result and 'depth_map' in result:
                ...
                depth_img_pil = depth_img_pil.crop((210, 0, 750, 540)).resize((512, 512))
                ...
                self.depth_img_buffer.append(PILImage.fromarray(depth_map.astype(np.float32)))
            ...
            if len(self.left_img_buffer) > 2:
                ...
                self.tip_position_camera_space_buffer = self.tip_position_camera_space_buffer[1:]

            if len(self.left_img_buffer) > 1:
                prediction = self.model.get_action(post=self.tip_position_camera_space_buffer, left_img=self.left_img_buffer,
    ...
    def get_depth_from_server(self, left_img, right_img):
        try:
            ...
            depth_map = np.frombuffer(
                response['depth_map'], 
                dtype=np.dtype(response['depth_dtype'])
    ...
    def move_robot_with_next_position(self, next_position_camera_space):
        ...
        if self.GLOBAL_ROBOT_POS is None and self.relative_tip_position_robot_space is not None:
        ...
        print(f"initial position of the episode in camera space {self.initial_tip_pos_camera_space}")

        if self.model.predict_global_position:
            # Re-add initial position, when we are predicting global position.
            next_position_camera_space = np.array(next_position_camera_space) + np.array(self.initial_tip_pos_camera_space)

        next_position_robot_space = camera_to_robot_space(next_position_camera_space, self.rotation)

        movement = np.array(next_position_robot_space) - np.array(self.GLOBAL_ROBOT_POS)
        
        print(f"{movement=}")
        if abs(movement[0]) > 0.05 or abs(movement[1]) > 0.05 or abs(movement[2]) > 0.05:
    ...
    def move_robot_with_action(self, action_camera_space):
        ...
        if abs(action_robot_space[0]) > 0.05 or abs(action_robot_space[1]) > 0.05 or abs(action_robot_space[2]) > 0.05:
