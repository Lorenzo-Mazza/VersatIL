"""Base inference client for model loading, validation, and inference."""
...
class BaseInferenceClient:
    """Loads a policy from a checkpoint and provides inference utilities.

    Handles trained policy configuration loading, checkpoint validation, tokenizer setup,
    ...
    def __init__(
        ...
        """Initialize base inference client.
    ...
    def _load_model(self) -> None:
        ...
        self._validate_checkpoint_loading(
            checkpoint["state_dict"], lightning_module
        )
    ...
    def _post_load_model(self) -> None:
        """Hook for subclasses after model loading. Default: no-op."""
    ...
    def _run_inference(
        ...
        with torch.autocast(
            ...
            dtype=MAP_PRECISION_TO_DTYPE[self.precision],
        ):
            with torch.no_grad():
    ...
    def prediction_horizon(self) -> int:
        ...
        return self.policy.prediction_horizon

    @property
    def observation_horizon(self) -> int:
        """Get the decoder's observation horizon."""
        return self.policy.decoder.observation_horizon