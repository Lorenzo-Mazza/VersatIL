"""Policy loading helpers for post-training compression."""
...
def load_float_policy_context(
    ...
    """Load a float policy checkpoint for compression."""
...
class QATPolicyLoader(BasePolicyLoader):
    """Load a torchao QAT checkpoint after preparing fake-quant modules."""
    ...
    def policy(self) -> Policy:
        """Get the loaded QAT-prepared policy."""
        return self._policy
...
def load_qat_policy_context(
    ...
    """Load a QAT checkpoint with fake-quant modules prepared first."""
    policy_loader = QATPolicyLoader(
        device=torch.device("cpu"),
        checkpoint_path=checkpoint_path,
        checkpoint_name=checkpoint_name,
