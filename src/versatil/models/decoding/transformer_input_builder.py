...
class TransformerInputBuilder(nn.Module):
    ...
    def forward(
        ...
        flat_tokens_list = []
        flat_mask_list = []
        cls_token, _cls_token_positional_encoding, cls_token_padding_mask = (
            None,
            None,
            None,
        )
        for name in sorted(projected.keys()):
            ...
            if x.ndim == 2:  # algorithm context (B, Emb), a single token
                ...
                is_spatial = True
                token_embeddings = (
                    x.flatten(3).transpose(2, 3).reshape(B, T * H * W, Emb)
                )  # (B, T*H*W, Emb)
            else:
            ...
            if (
                ...
                and action_padding_mask is not None
            ):
            ...
            if is_spatial and self.spatial_positional_encoding_layer is not None:
                pe_2d = self.spatial_positional_encoding_layer(
                ...
                if self.camera_embeddings is not None:
                    pe = pe + self.camera_embeddings(
                        name, x.device
                    )  # extract camera embeddings with correct key and add to PE
        ...
        spatial_tokens = (
            torch.cat(spatial_tokens_list, dim=1) if spatial_tokens_list else None
        )  # (B, L_spatial, Emb)
        flat_tokens = (
            torch.cat(flat_tokens_list, dim=1) if flat_tokens_list else None
        )  # (B, L_flat, Emb)
        tokens = torch.cat(
            [t for t in [spatial_tokens, flat_tokens] if t is not None], dim=1
        )  # (B, Total_Seq, Emb)
        ...
        if self.spatial_positional_encoding_layer is None:
            ...
            if self.flat_positional_encoding_layer is not None:
                ...
                positional_encodings = positional_encodings.expand(
                    B, -1, -1
                )  # (B, Total_Seq, Emb)
            else:
            ...
            spatial_positional_encodings = (
                torch.cat(spatial_positional_encodings, dim=1)
                if spatial_positional_encodings
                else None
            )
            ...
            if flat_tokens is not None:
                if self.flat_positional_encoding_layer is not None:
                    ...
                    flat_positional_encodings = flat_positional_encodings.expand(
                        B, -1, -1
                    )  # (B, L_flat, Emb)
                else:
            ...
            pe_list = [
                p
                for p in [spatial_positional_encodings, flat_positional_encodings]
                if p is not None
            ]
            positional_encodings = (
                torch.cat(pe_list, dim=1) if pe_list else None
            )  # (B, Total_Seq, Emb)

        padding_mask = torch.cat(
            spatial_mask_list + flat_mask_list, dim=1
        )  # (B, Total_Seq)
