...
Reference implementation based on DiT architecture by Peebles and Xie.
"""
...
class DiTDecoder(ActionDecoder):
    """DiT (Diffusion Transformer) decoder for action generation.
    ...
    Note:
        This decoder is specifically designed for denoising score matching algorithms
    ...
    def __init__(
        ...
        Args:
            ...
            number_of_key_value_heads: Number of Key/Value heads for Group Query Attention (None = same as heads = Multi-Head Attention)
            ...
            use_gating: Whether to use gating in modulation layers, i.e. AdaLN-Zero layer
        """
        ...
        for k, head in action_heads.items():
            if len(head.blocks) > 0:
                logging.warning(
                    f"Action heads are ignored by DiTDecoder, but one was provided for action '{k}'. Skipping."
                )
    ...
    def _build_transformer_components(self):
        ...
        self._encoder_cache: Optional[list[torch.Tensor]] = None  # Encoder cache for inference optimization
        self.to(self.device)
    ...
    def _prepare_observation_tokens(
        ...
        Returns:
            Tuple of (observation_tokens, positional_encodings, observation_padding_mask) where:
                ...
                - observation_padding_mask: (batch_size, num_tokens) or None, where True indicates padding
        Raises:
    ...
    def forward(
        ...
        This method is called by the decoding algorithm which needs to provide:
        - Noisy actions
        - Observation features dictionary containing the timestep key

        Args:
            features: Dictionary of encoded features from the encoding pipeline plus the timestep.
        ...
        if actions is None:
            raise ValueError(
                "DiTDecoder requires 'actions' parameter. "
        ...
        noisy_actions = torch.cat(action_tensors, dim=-1)  # (B, prediction_horizon, total_action_dimension)
        noisy_embedding = self.noisy_input_projection(noisy_actions)  # (B, prediction_horizon, embedding_dimension)
        if self.training:
            self._encoder_cache = None  # Clear any cached encoder outputs during training
        ...
        )  # (B, prediction_horizon, total_action_dimension)

        if not self.training:
            self._encoder_cache = encoder_cache  # Cache encoder output for inference optimization
        outputs = {}
        start_index = 0
        for action_key in sorted(actions.keys()):
            head = self.action_heads[action_key]
            end_index = start_index + head.output_dim
            action_slice = noise_predictions[..., start_index:end_index]  # (B, T, action_dimension_i)
