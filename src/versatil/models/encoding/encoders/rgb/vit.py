...
class ViTEncoder(Encoder):
    ...
    def _setup_feature_extractor(self):
        ...
        if self.pooling_method == PoolingMethod.LEARNED_AGGREGATION.value:
            self.pooling_head = LearnedAggregation(
                self.feature_dim - 1
            )  # feature_dim-1 to exclude CLS token
        if self.pooling_method == PoolingMethod.NONE.value:
            self.output_dim = (
                -1,
                self.feature_dim - 1,
            )  # -1 indicates the variable seq dimension, feature_dim-1 to exclude CLS token
        else:
    ...
    def forward(self, inputs: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if has_time:
            features = features.reshape(
                B, T, *features.shape[1:]
            )  # B, T, Emb or B, T, Seq, Emb
