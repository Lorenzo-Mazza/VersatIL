...
**Workaround:** `src/versatil/quantization/torch_patches.py` (`register_torchao_patches`) installs an import hook that rewrites the crashing `__module__` assignments to `pass` in memory as the torchao modules load. Registered automatically when `versatil` is imported.
...
**Workaround:** `src/versatil/quantization/torch_patches.py` (`register_torchao_patches`) installs an import hook that rewrites the QAT fake-quant config in memory as the module loads, so it uses `base_config.group_size`. Wheels that already include the upstream fix are accepted unchanged; unknown torchao source raises instead of silently misquantizing.
